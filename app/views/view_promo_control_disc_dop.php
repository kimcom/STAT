...
//************************************//
	...
	$.post('../lists/get_promo_list',{promo_type:'50'},function(json){
	...
	$("#select_promo_list").click(function () { 
		...
		$("#grid1").jqGrid('setGridParam',{url:"../goods/list?col=disc_dop&param=in promo_goods_item&cat_id="+row_id,page:1});
		$("#grid1").trigger('reloadGrid');
		$("#grid2").jqGrid('setGridParam',{url:"../goods/list?col=disc_dop&param=no promo_goods_item&cat_id="+row_id,page:1});
		...
		$("#grid5").jqGrid('setGridParam',{url:"../goods/get_cats_list?param=in promo_cats_action&cat_id="+row_id,page:1});
		$("#grid5").trigger('reloadGrid');
		$("#grid6").jqGrid('setGridParam',{url:"../goods/get_cats_list?param=no promo_cats_action&cat_id="+row_id,page:1});
		$("#grid6").trigger('reloadGrid');
	});
...
// Creating grid1
	$("#grid1").jqGrid({
		...
		datatype: "json",
		height: '230px',
		colNames:['Артикул','Название','Опт.цена','Макс.скидка'],
		colModel:[
...
// Creating grid5
	$("#grid5").jqGrid({
		...
		toppager: true,
		hiddengrid: true,
		caption: "Список категорий на которые действует акционная скидка:",
		pager: '#pgrid5',
		beforeSaveCell: function(rowid,cellname,value,iRow,iCol) { 
			$("#grid5").jqGrid('setGridParam', {cellurl: "../goods/edit_discount_cat2?promoid=" + $("#PromoID").val()});
        },
		cellEdit: true,
		cellsubmit: 'remote'
	});
	$("#grid5").jqGrid('navGrid','#pgrid5', {edit:false, add:false, del:false, search:false, refresh: true, cloneToTop: true});
	$("#grid5").jqGrid('filterToolbar', { autosearch: true,	searchOnEnter: true	});

	$("#grid5").navButtonAdd('#grid5_toppager',{
		title:'Удалить из акции', buttonicon:"ui-icon-minusthick", caption:'из акции', position:"last",
		onClickButton: function(){ 
			id=$("#select_promo_list").select2("val");
			if(id==null){
			...
			var sel;
			sel = jQuery("#grid5").jqGrid('getGridParam','selarrrow');
			if(sel==''){
				$("#dialog>#text").html('Вы не выбрали ни одной записи!');
				$("#dialog").dialog( "open" );
				return;
			...
			$("#grid5").jqGrid('delGridRow', id, {
				modal:true,
				closeOnEscape:true,
				closeAfterDel:true,
				reloadAfterSubmit: true,
				msg: 'Удалить выбранные товары из акции<br/>'+$("#select_promo_list").select2("data").text+'?',
				url: '../goods/del_cats_from_promo2?promo_id='+id+'&source='+sel,
				savekey : [ true, 13 ],
				afterSubmit : function(json, postdata) {
					var result=$.parseJSON(json.responseText);
					if(result.success)$("#grid6").trigger("reloadGrid");
					return [result.success,result.message,result.new_id];
				}
				} 
			);
	...
	$("#pg_pgrid5").remove();
	$("#pgrid5").removeClass('ui-jqgrid-pager');
	$("#pgrid5").addClass('ui-jqgrid-pager-empty');
...
// Creating grid6
	...
	$("#grid6").jqGrid('navGrid','#pgrid6', {edit:false, add:false, del:false, search:false, refresh: true,	cloneToTop: true});
	$("#grid6").jqGrid('filterToolbar', { autosearch: true,	searchOnEnter: true	});

	$("#grid6").navButtonAdd('#grid6_toppager',{
		title:'Добавить в акцию', buttonicon:"ui-icon-plusthick", caption:'в акцию', position:"last",
		onClickButton: function(){ 
			id=$("#select_promo_list").select2("val");
			if(id==null){
			...
			var sel;
			sel = jQuery("#grid6").jqGrid('getGridParam','selarrrow');
			if(sel==''){
				$("#dialog>#text").html('Вы не выбрали ни одной записи!');
				$("#dialog").dialog( "open" );
				return;
			}
			$.post('../goods/cats_add_in_promo2?promo_id='+id+'&source='+sel,function(data){
				if(data==0){
					$("#dialog>#text").html('Возникла ошибка.<br/>Сообщите разработчику!');
					$("#dialog").dialog( "open" );
				}else{
	...
	$("#pg_pgrid6").remove();
	$("#pgrid6").removeClass('ui-jqgrid-pager');
	$("#pgrid6").addClass('ui-jqgrid-pager-empty');
	...
	$("#grid5").jqGrid('bindKeys', {"onEnter":function( rowid ) { alert("You enter a row with id:"+rowid)} } );
	$("#grid6").jqGrid('bindKeys', {"onEnter":function( rowid ) { alert("You enter a row with id:"+rowid)} } );
...
<div class="container-fluid min570">
	...
	<div style='display:block;' class='frameL'>
	...
	<div style='display:block;' class='frameL'>
		<div id='div5' class='frameL pt5'>
			<table id="grid5"></table>
			<div id="pgrid5"></div>
		</div>
		<div id='div6' class='frameL pt5 pl10'>
