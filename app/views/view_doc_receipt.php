...
//		}
	...
	$("#div_doc_list button").click(function(e){
		...
		if (id == 'doc_edit') {
			...
			$.post('/engine/doc_edit', {action: 'receipt_setcurrent',docid:id,operid:<?php echo $operid ?>}, function (json) {
			//console.log(json);
			    if (!json.success) {
					$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
		...
		if (id == 'doc_view')	{
			...
			$.post('/engine/doc_info_full',{action: 'receipt_info', docid: id, view: true}, function (json) {
				if (json.success){
					$("#view").html(json.html);
		...
		if (id == 'doc_add') {
		    $.post('/engine/doc_edit', {action: 'receipt_new', operid:<?php echo $operid;?>}, function (json) {
				//console.log(json);
				if (!json.success) {
				    $("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				    $("#dialog>#text").html(json.message);
	...
	doc_info = function () {
		$.post('/engine/doc_info_full',{action: 'receipt_info'}, function (json) {
			//console.log(json);
			if (json.success){
				...
				$.post('/engine/select2?action=point', function (json) {
					...
					$("#select_companyID").on("change", function (e) {
					    if (e.val.length > 0)
						good_edit('receipt_edit_client', null, 0, 0, 0, 0, 0, 0, e.val);
					});
				...
				$.post('/engine/select2?action=partner', function (json) {
					...
					$("#select_partnerID").on("change", function (e) {
					    if (e.val.length > 0)
						good_edit('receipt_edit_partner', null, 0, 0, 0, 0, 0, 0, 0, 0, e.val);
					});
				...
				$("#div_doc_buttons button").click(function(e){
					...
					if (id == 'good_add') {window.location.href='/main/catalog?oper=receipt&operid=<?php echo $operid?>';}
	...
	doc_send = function (e) {
		$(this).dialog("close");
		$.post('/engine/doc_edit', {action: 'receipt_send',operid:<?php echo $operid ?>}, function (json) {
			//console.log(json);
			if (!json.success) {
				$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				$("#dialog>#text").html(json.message);
	...
	doc_delete = function (e) {
		$(this).dialog("close");
		$.post('/engine/doc_edit', {action: 'receipt_delete'}, function (json) {
			//console.log(json);
			if (!json.success) {
				$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				$("#dialog>#text").html(json.message);
	...
	doc_delete_from_list = function (e) {
		...
		$.post('/engine/doc_edit', {action: 'receipt_delete', docid: id}, function (json) {
			//console.log(json);
			if (!json.success) {
				$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				$("#dialog>#text").html(json.message);
	...
	good_edit = function (action, el, goodid, size, qty, info, delivery, notes, newclientid, typepay, partnerid, invoice) {
		...
		$.post('/engine/doc_edit', {action: action, operid:<?php echo $operid ?>, goodid: goodid, modifiers:size, qty: qty, info: info, delivery: delivery, notes: notes, clientid: newclientid, typepay:typepay, partnerid:partnerid, invoice:invoice}, function (json) {
		    ...
		    if (json.success && action == 'receipt_edit' && qty == 0) {
		...
		$.post('/engine/client_edit', {action: 'client_edit',clientid:null, name: client_name, phone: client_phone}, function (json) {
			...
			if (!json.success) {
				...
				$.post('/engine/select2?action=partners', function (json) {
					...
					good_edit('receipt_edit_client', null, 0, 0, 0, 0, 0, 0, id);
			    });
			}
		});
...
?>
	<ul id="myTab" class="nav nav-tabs floatL active hidden-print" role="tablist">
		<li>				<a id="a_tab_0" href="#tab_doc_action" role="tab" data-toggle="tab"><?php echo ($operid==1?'Приход':'Возврат')?> товара</a></li>
		<li class="active">	<a id="a_tab_1" href="#tab_doc_list1"  state="0" role="tab" data-toggle="tab">Непроведенные</a></li>
		<li>				<a href="#tab_doc_list2"	role="tab" state="1" data-toggle="tab">Проведенные</a></li>
