...
$(document).ready(function () {
	...
	$("#grid1").jqGrid({
		...
		mtype: "GET",
		styleUI: 'Bootstrap',
		url: "/engine/jqgrid3?action=timesheet_list&grouping=DocID&o.Status=0&f1=DocID&f2=State&f3=DT_create&f4=Qty&f5=ClientName&f6=Notes&f7=Author",
		//responsive: true,
...
//		}
	...
	$('#myTab a').click(function (e) {
		...
		if ($(this).attr('state')) {
			...
			if($(this).attr('state')=='all') {
				$("#grid1").jqGrid('setGridParam', {datatype: "json", url: "/engine/jqgrid3?action=timesheet_list&grouping=DocID&f1=DocID&f2=State&f3=DT_create&f4=Qty&f5=ClientName&f6=Notes&f7=Author", page: 1});
			}else{
				$("#grid1").jqGrid('setGridParam', {datatype: "json", url: "/engine/jqgrid3?action=timesheet_list&grouping=DocID&o.Status="+$(this).attr('state')+"&f1=DocID&f2=State&f3=DT_create&f4=Qty&f5=ClientName&f6=Notes&f7=Author", page: 1});
			}
	...
	$("#div_doc_list button").click(function(e){
		...
		if (id == 'doc_edit') {
			...
			$.post('/engine/doc_edit', {action: 'timesheet_setcurrent',docid:id}, function (json) {
			//console.log(json);
			    if (!json.success) {
					$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
		...
		if (id == 'doc_view')	{
			...
			$.post('/engine/doc_info_full',{action: 'timesheet_info', docid: id, view: true}, function (json) {
				if (json.success){
					$("#view").html(json.html);
		...
		if (id == 'doc_add') {
		    $.post('/engine/doc_edit', {action: 'timesheet_new'}, function (json) {
				//console.log(json);
				if (!json.success) {
				    $("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				    $("#dialog>#text").html(json.message);
	...
	doc_info = function () {
		$.post('/engine/doc_info_full',{action: 'timesheet_info'}, function (json) {
			//console.log(json);
			if (json.success){
				...
				$.post('/engine/select2?action=point', function (json) {
					...
					$("#select_companyID").on("change", function (e) {
					    if (e.val.length > 0)
						good_edit('timesheet_edit_client', null, 0, 0, 0, 0, 0, 0, e.val);
					});
				...
				dt = new Date();
				if (dt.getMonth()==0) {
					dt = new Date(dt.getFullYear()-1,11,1);
					obj = new Object();
					obj['id'] = ''+(dt.getFullYear())+'-12';
					obj['text'] = ''+dt.toLocaleString("ru",{month: 'long'})+' '+(dt.getFullYear());
					period[0] = obj;
				}
				dt = new Date();
				year = dt.getFullYear();
				for (i=1;i<=12;i++){
					dt1 = new Date(year,i-1,1);
					var d = '0'+i; d = d.substring(d.length-2);
					obj = new Object();
					obj['id'] = year + '-' + d;
					obj['text'] = ''+dt1.toLocaleString("ru",{month: 'long'})+' '+dt1.getFullYear();
					period[period.length] = obj;
				}
				$("#select_periodID").select2({enable: false, multiple: false, placeholder: "Укажите период", data: {results: period, text: 'text'}});
				$("#select_periodID").select2("val", json.period);
				$("#select_periodID").on("change", function (e) {
					if (e.val.length > 0)
					good_edit('timesheet_edit_period', null, 0, 0, 0, e.val, 0, 0, 0);
				});
				
				$("#doc_fill").click(function(e){
					if($("#select_companyID").val()=='') {
						$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
						$("#dialog>#text").html("Вы не указали магазин!");
						$("#dialog").dialog('open');
					}else if($("#select_periodID").val()=='') {
						$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
						$("#dialog>#text").html("Вы не указали период расчета!");
						$("#dialog").dialog('open');
					}else{
						$("#question>#text").html("Текущие данные будут удалены!<br><br>Заполнить документ № " + $('#docid').val() + "?");
						$("#question").dialog('option', 'buttons', [{text: "Отправить", click: doc_fill}, {text: "Отмена", click: function () {$(this).dialog("close");}}]);
						$("#question").dialog('open');
					}
				});
				$("#div_doc_buttons button").click(function(e){
					...
					if (id == 'good_add') {window.location.href='/main/catalog?oper=timesheet&operid=0';}
					if (id == 'doc_add') {$("#div_doc_list #doc_add:first").click();}
				});
			}
		});
	}
	doc_cell = function(id){
		el = $('#'+id);
		sellerid = id.split('_')[0];
		dt = id.split('_')[1];
		//console.log(sellerid,dt);
		if($(el).hasClass('freeday')){
			val = 'О';
			$(el).removeClass('freeday');
			$(el).addClass('holiday');
		}else if($(el).hasClass('holiday')){
			val = 'Б';
			$(el).removeClass('holiday');
			$(el).addClass('hospital');
		}else if($(el).hasClass('hospital')){
			val = '8';
			$(el).removeClass('hospital');
			$(el).addClass('workday');
		}else if($(el).hasClass('workday') || $(el).hasClass('orangeday')){
			val = 'Х';
			$(el).removeClass('orangeday');
			$(el).removeClass('workday');
			$(el).addClass('noworking');
		}else if($(el).hasClass('noworking')){
			val = 'В';
			$(el).removeClass('noworking');
			$(el).addClass('freeday');
		}
		$(el).html(val);
		good_edit('timesheet_edit_cell',el,sellerid, 0, 0, val,'',dt,0,0,0)
	}
	doc_fill = function (e) {
		$(this).dialog("close");
		$.post('/engine/doc_edit', {action: 'timesheet_fill', clientid: $("#select_companyID").val()}, function (json) {
			if (!json.success) {
				$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				$("#dialog>#text").html(json.message);
				$("#dialog").dialog("open");
	...
	doc_send = function (e) {
		$(this).dialog("close");
//		if ($("#select_companyID").select2("val")=="") msg = "Не указан магазин!";
//		if ($("#select_companyID").select2("val")=="") msg = "Не указан магазин!";
//		return;
		$.post('/engine/doc_edit', {action: 'timesheet_send'}, function (json) {
			//console.log(json);
			if (!json.success) {
				$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				$("#dialog>#text").html(json.message);
	...
	doc_delete = function (e) {
		$(this).dialog("close");
		$.post('/engine/doc_edit', {action: 'timesheet_delete'}, function (json) {
			//console.log(json);
			if (!json.success) {
				$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				$("#dialog>#text").html(json.message);
	...
	doc_delete_from_list = function (e) {
		...
		$.post('/engine/doc_edit', {action: 'timesheet_delete', docid: id}, function (json) {
			//console.log(json);
			if (!json.success) {
				$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				$("#dialog>#text").html(json.message);
	...
	good_edit = function (action, el, goodid, size, qty, info, delivery, notes, newclientid, typepay, partnerid) {
		...
		$.post('/engine/doc_edit', {action: action, goodid: goodid, modifiers:size, qty: qty, info: info, delivery: delivery, notes: notes, clientid: newclientid, typepay:typepay, partnerid:partnerid }, function (json) {
		    ...
		    if (json.success && action == 'timesheet_edit' && qty == 0) {
		...
		$.post('/engine/client_edit', {action: 'client_edit',clientid:null, name: client_name, phone: client_phone}, function (json) {
			...
			if (!json.success) {
				...
				$.post('/engine/select2?action=partners', function (json) {
					...
					good_edit('timesheet_edit_client', null, 0, 0, 0, 0, 0, 0, id);
			    });
			}
		});
...
?>
	<ul id="myTab" class="nav nav-tabs floatL active hidden-print" role="tablist">
		<li>				<a id="a_tab_0" href="#tab_doc_action" role="tab" data-toggle="tab">Табель</a></li>
