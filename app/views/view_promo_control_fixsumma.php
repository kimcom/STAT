...
//************************************//
	...
	$.post('../lists/get_promo_list',{promo_type:'60'},function(json){
...
// Creating grid1
	$("#grid1").jqGrid({
		...
		beforeSaveCell: function(rowid,cellname,value,iRow,iCol) { 
			$("#grid1").jqGrid('setGridParam', {cellurl: "post_goods.php?action=edit_discount&promoid=" + $("#PromoID").val()});
        },
		cellEdit: true,
		cellsubmit: 'remote'
	});
	...
	$("#grid1").navButtonAdd('#grid1_toppager',{
		...
		onClickButton: function(){ 
			...
			$("#grid1").jqGrid('delGridRow', id, {
				...
				url: 'post_goods.php?button=del_goods_from_promo&promo_id='+id+'&source='+sel,
				savekey : [ true, 13 ],
...
// Creating grid2
	...
	$("#grid2").navButtonAdd('#grid2_toppager',{
		...
		onClickButton: function(){ 
			...
			$.post('post_goods.php?oper=goods_add_in_promo&promo_id='+id+'&source='+sel,function(data){
				if(data==0){
