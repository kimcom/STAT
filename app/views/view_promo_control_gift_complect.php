...
//************************************//
	...
	$.post('../lists/get_promo_list',{promo_type:'80'},function(json){
...
// Creating grid3
	$("#grid3").jqGrid({
		sortable: true,
		//url:"post_goods.php?action=get_goods_list&param=in promo_goods_action&cat_id=-1",
		datatype: "json",
		height: '230px',
		...
		toppager: true,
		hiddengrid: true,
		caption: "Список товаров предлагаемых в подарок:",
		pager: '#pgrid3'
	});
	...
	$("#grid3").navButtonAdd('#grid3_toppager',{
		...
		onClickButton: function(){ 
			...
			$("#grid3").jqGrid('delGridRow', id, {
				...
				reloadAfterSubmit: true,
				msg: 'Удалить выбранные товары из акции<br/>'+$("#select_promo_list").select2("data").text+'?',
				url: '../goods/del_goods_from_promo_action?promo_id='+id+'&source='+sel,
				savekey : [ true, 13 ],
				afterSubmit : function(json, postdata) {
					var result=$.parseJSON(json.responseText);
...
// Creating grid4
	$("#grid4").jqGrid({
		sortable: true,
		//url:"post_goods.php?action=get_goods_list&param=no promo_goods_action&cat_id=-1",
		datatype: "json",
		mtype: "POST",
		height: '230px',
		colNames:['Артикул','Название'],
		...
		toppager: true,
		hiddengrid: true,
		caption: "Список товаров для выбора подарков:",
		pager: '#pgrid4'
	});
	...
	$("#grid4").navButtonAdd('#grid4_toppager',{
		...
		onClickButton: function(){ 
			...
			$.post('../goods/goods_add_in_promo_action?promo_id='+id+'&source='+sel,function(data){
				if(data==0){
					$("#dialog>#text").html('Возникла ошибка.<br/>Сообщите разработчику!');
					$("#dialog").dialog( "open" );
				}else{
