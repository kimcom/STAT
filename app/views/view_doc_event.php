...
$(document).ready(function () {
	...
	$("#div_task").dialog({
		...
		open: function (event, ui) {
		    $("[aria-describedby=div_task] > .ui-dialog-titlebar").remove();
		}
    });
	...
	$("#grid1").jqGrid({
		...
		mtype: "GET",
		styleUI: 'Bootstrap',
		url: "/engine/jqgrid3?action=event_list&grouping=DocID&o.Status=0&f1=DocID&f2=State&f3=DT_create&f4=Sum&f5=Partner&f6=Description&f7=Author",
		//responsive: true,
		...
		colModel: [
		    ...
		    {label: 'Партнер', name: 'Partner', index: 'Partner', width: 200, sorttype: "text", search: true, align: "left"},
...
//		forceFit:true,
	...
	$('#myTab a').click(function (e) {
		...
		if ($(this).attr('state')) {
			...
			if($(this).attr('state')=='all') {
				$("#grid1").jqGrid('setGridParam', {datatype: "json", url: "/engine/jqgrid3?action=event_list&grouping=DocID&f1=DocID&f2=State&f3=DT_create&f4=Sum&f5=Partner&f6=Description&f7=Author", page: 1});
			}else{
				$("#grid1").jqGrid('setGridParam', {datatype: "json", url: "/engine/jqgrid3?action=event_list&grouping=DocID&o.Status="+$(this).attr('state')+"&f1=DocID&f2=State&f3=DT_create&f4=Sum&f5=Partner&f6=Description&f7=Author", page: 1});
			}
	...
	$("#div_doc_list button").click(function(e){
		...
		if (id == 'doc_edit') {
			...
			$.post('/engine/event_edit', {action: 'event_setcurrent', docid:id, operid:<?php echo $operid ?>}, function (json) {
			//console.log(json);
			    if (!json.success) {
					$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
		...
		if (id == 'doc_view')	{
			...
			$.post('/engine/doc_info_full',{action: 'event_info', docid: id, view: true}, function (json) {
				if (json.success){
					$("#view").html(json.html);
		...
		if (id == 'doc_add') {
		    $.post('/engine/doc_edit', {action: 'event_new', operid:<?php echo $operid;?>}, function (json) {
				//console.log(json);
				if (!json.success) {
				    $("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				    $("#dialog>#text").html(json.message);
	...
	doc_info = function () {
		$.post('/engine/event_info_full',{action: 'event_info'}, function (json) {
			//console.log(json);
			if (json.success){
				...
				$("#div_doc_buttons button").click(function(e){
					...
					if (id == 'good_add') {window.location.href='/main/catalog?oper=event&operid=0';}
	...
	doc_send = function (e) {
		$(this).dialog("close");
		$.post('/engine/event_edit', {action: 'event_send'}, function (json) {
			//console.log(json);
			if (!json.success) {
				$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				$("#dialog>#text").html(json.message);
	...
	doc_delete = function (e) {
		$(this).dialog("close");
		$.post('/engine/event_edit', {action: 'event_delete'}, function (json) {
			//console.log(json);
			if (!json.success) {
				$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				$("#dialog>#text").html(json.message);
	...
	doc_delete_from_list = function (e) {
		...
		$.post('/engine/event_edit', {action: 'event_delete', docid: id}, function (json) {
			//console.log(json);
			if (!json.success) {
				$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				$("#dialog>#text").html(json.message);
	...
	good_edit = function (action, el, goodid, size, qty, info, delivery, notes, newclientid, typepay) {
		...
		$.post('/engine/doc_edit', {action: action, operid:<?php echo $operid ?>, goodid: goodid, modifiers:size, qty: qty, info: info, delivery: delivery, notes: notes, clientid: newclientid, typepay:typepay}, function (json) {
		    ...
		    if (json.success && action == 'event_edit' && qty == 0) {
	...
	client_new = function (){
		$("#div_task").dialog("open");
	}
	$('#div_task #btn_client_new').click(function(e){
		client_name = $("#div_task #client_name").val();
		client_phone = $("#div_task #client_phone").val();
		if (client_name==''){
			$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
		...
		$.post('/engine/client_edit', {action: 'client_edit',clientid:null, name: client_name, phone: client_phone}, function (json) {
			...
			if (!json.success) {
				...
				id = json.id;
				$("#div_task").dialog("close");
				$("#div_task #client_name").val('');
				$("#div_task #client_phone").val('');
				$.post('/engine/select2?action=partners', function (json) {
					...
					$("#select_companyID").on("change", function (e) {
					    if (e.val.length > 0)
						good_edit('event_edit_client', null, 0, 0, 0, 0, 0, 0, e.val);
					});
					$("#select_companyID").select2("val", id);
					good_edit('event_edit_client', null, 0, 0, 0, 0, 0, 0, id);
			    });
			}
		});
...
?>
	<ul id="myTab" class="nav nav-tabs floatL active hidden-print" role="tablist">
		<li>				<a id="a_tab_0" href="#tab_doc_action" role="tab" data-toggle="tab">Документ "Событие"</a></li>
		...
		<li>				<a id="a_tab_3" href="#tab_doc_list3"	role="tab" state="all" data-toggle="tab">Все документы</a></li>
	</ul>
...
<div id="div_task" style="padding: 0;z-index: 2000;">
