...
//************************************//
	$("#treegrid").jqGrid({
		...
		//sortable: true,
		sortorder: "asc",
		editurl: '../category/category_tree_oper',
		pager : "#ptreegrid",
		caption: "Категории",
		toppager: true,
		...
		onSelectRow: function(cat_id) {
			...
			//$("#grid4").jqGrid('setGridParam',{url:"post_goods.php?action=get_goods_list&col=cat&param=not in category&cat_id="+cat_id,page:1});
	...
	$("#treegrid").navButtonAdd('#treegrid_toppager',{
		...
		onClickButton: function(){ 
			...
			if(source_id_cut){
				...
				$.post('../category/category_tree_oper?oper=move&source='+source_id_cut+'&target='+target_id,function(data){
					if(data==0){
			...
			if(source_id_copy){
				...
				$.post('../category/category_tree_oper?oper=copy&source='+source_id_copy+'&target='+target_id,function(data){
					if(data==0){
						$("#dialog>#text").html('Возникла ошибка.<br/>Сообщите разработчику!');
						$("#dialog").dialog( "open" );
					}else{
...
//***** treegrid2 *********//
	$("#treegrid2").jqGrid({
		...
		onSelectRow: function(cat_id) {
			if(cat_id == null) cat_id=0;
			//$("#grid1").jqGrid('setGridParam',{url:"post_goods.php?action=get_goods_list&col=cat&param=in category&cat_id="+cat_id,page:1});
			//$("#grid1").trigger('reloadGrid');
			$("#grid4").jqGrid('setGridParam',{url:"../goods/list?col=cat&param=in category&cat_id="+cat_id,page:1});
			$("#grid4").trigger('reloadGrid');
		}
	});
	...
	$("#grid1").navButtonAdd('#grid1_toppager',{
		...
		onClickButton: function(){ 
			...
			$("#grid1").jqGrid('delGridRow', id, {
				...
				closeAfterDel:true,
				reloadAfterSubmit: true,
				msg: 'Удалить выбранные товары из категории<br/>'+node.name+'?',
