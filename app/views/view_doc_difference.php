...
$(document).ready(function () {
	...
	$("#grid1").jqGrid({
		...
		mtype: "GET",
		styleUI: 'Bootstrap',
		url: "/engine/jqgrid3?action=difference_list&grouping=DocID&o.Status=0&f1=DocID&f2=State&f3=DT_create&f4=Qty&f5=ClientName&f6=Notes&f7=Author",
		//responsive: true,
...
//		}
	...
	$('#myTab a').click(function (e) {
		...
		if ($(this).attr('state')) {
			...
			if($(this).attr('state')=='all') {
				$("#grid1").jqGrid('setGridParam', {datatype: "json", url: "/engine/jqgrid3?action=difference_list&grouping=DocID&f1=DocID&f2=State&f3=DT_create&f4=Qty&f5=ClientName&f6=Notes&f7=Author", page: 1});
			}else{
				$("#grid1").jqGrid('setGridParam', {datatype: "json", url: "/engine/jqgrid3?action=difference_list&grouping=DocID&o.Status="+$(this).attr('state')+"&f1=DocID&f2=State&f3=DT_create&f4=Qty&f5=ClientName&f6=Notes&f7=Author", page: 1});
			}
	...
	$("#div_doc_list button").click(function(e){
		...
		if (id == 'doc_edit') {
			...
			$.post('/engine/doc_edit', {action: 'difference_setcurrent',docid:id}, function (json) {
			//console.log(json);
			    if (!json.success) {
					$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
		...
		if (id == 'doc_view')	{
			...
			$.post('/engine/doc_info_full',{action: 'difference_info', docid: id, view: true}, function (json) {
				if (json.success){
					$("#view").html(json.html);
		...
		if (id == 'doc_add') {
		    $.post('/engine/doc_edit', {action: 'difference_new'}, function (json) {
				//console.log(json);
				if (!json.success) {
				    $("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				    $("#dialog>#text").html(json.message);
	...
	doc_info = function () {
		$.post('/engine/doc_info_full',{action: 'difference_info'}, function (json) {
			//console.log(json);
			if (json.success){
				...
				$.post('/engine/select2?action=point', function (json) {
					...
					$("#select_companyID").on("change", function (e) {
					    if (e.val.length > 0)
						good_edit('difference_edit_client', null, 0, 0, 0, 0, 0, 0, e.val);
					});
					$("#select_companyID").select2("val", clientid);
					//$("#select_companyID").select2("enable", mode_manager);
			    });
				$.post('/engine/select2?action=partner', function (json) {
					$("#select_partnerID").select2({enable: false, multiple: false, placeholder: "Укажите контрагента", data: {results: json, text: 'text'}});
					$("#select_partnerID").on("change", function (e) {
					    if (e.val.length > 0)
						good_edit('difference_edit_partner', null, 0, 0, 0, 0, 0, 0, 0, 0, e.val);
					});
					$("#select_partnerID").select2("val", partnerid);
			    });
				$("#div_doc_buttons button").click(function(e){
					...
					if (id == 'good_add') {window.location.href='/main/catalog?oper=difference&operid=0';}
	...
	doc_send = function (e) {
		$(this).dialog("close");
		$.post('/engine/doc_edit', {action: 'difference_send'}, function (json) {
			//console.log(json);
			if (!json.success) {
				$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				$("#dialog>#text").html(json.message);
	...
	doc_delete = function (e) {
		$(this).dialog("close");
		$.post('/engine/doc_edit', {action: 'difference_delete'}, function (json) {
			//console.log(json);
			if (!json.success) {
				$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				$("#dialog>#text").html(json.message);
	...
	doc_delete_from_list = function (e) {
		...
		$.post('/engine/doc_edit', {action: 'difference_delete', docid: id}, function (json) {
			//console.log(json);
			if (!json.success) {
				$("#dialog").css('background-color', 'linear-gradient(to bottom, #f7dcdb 0%, #c12e2a 100%)');
				$("#dialog>#text").html(json.message);
	...
	good_edit = function (action, el, goodid, size, qty, info, delivery, notes, newclientid, typepay, partnerid, invoice) {
		//console.log(action, el, goodid, size, qty, info);
		$.post('/engine/doc_edit', {action: action, goodid: goodid, modifiers:size, qty: qty, info: info, delivery: delivery, notes: notes, clientid: newclientid, typepay:typepay, partnerid:partnerid, invoice:invoice}, function (json) {
		    ...
		    if (json.success && action == 'difference_edit' && qty == 0) {
		...
		$.post('/engine/client_edit', {action: 'client_edit',clientid:null, name: client_name, phone: client_phone}, function (json) {
			...
			if (!json.success) {
				...
				$.post('/engine/select2?action=partners', function (json) {
					...
					good_edit('difference_edit_client', null, 0, 0, 0, 0, 0, 0, id);
			    });
			}
		});
...
?>
	<ul id="myTab" class="nav nav-tabs floatL active hidden-print" role="tablist">
		<li>				<a id="a_tab_0" href="#tab_doc_action" role="tab" data-toggle="tab">Акт разногласий</a></li>
