	...
	$('#fileupload').fileupload({
		...
		maxNumberOfFiles:1,
		    ...
		    $.each(data.result.files, function (index, file) {
				$.post('../engine/rename_image', {
					new_name: $("#GoodID").html(),
					old_name: file.name,
					img_good: true,
				},
				function (data) {
					if(!data.success){
						$("#dialog>#text").html(data.error);
						$("#dialog").dialog("open");
					} else {
						$('#img_good').attr('src',data.url+"?nd=" + new Date().getTime());
					}
				});
		    });
		},
...
<div class="container-fluid center">
    <ul id="myTab" class="nav nav-tabs floatL active hidden-print ml15" role="tablist">
        <li class="active">
    ...
    <div class="tab-content ml15">
        <div class="active tab-pane min530 m0 w100p ui-corner-tab1 borderTop1 borderColor frameL border1" id="tab_filter">
            <div class='p5 ui-corner-all frameL border0 w600' style='display:table;'>
                ...
                <div class="input-group input-group-sm w100p">
                    <span class="input-group-addon w130 TAL">Описание</span>
					<textarea id="description" name="description" rows="7" type="text" style="height: 100px;" class="form-control TAL p5"><?php echo $row['Description']; ?></textarea>
                    <span class="input-group-addon w32"></span>
                </div>
				<input id="imageURL"  type="text"  class="form-control TAR hidden" value="<?php echo $row['ImageURL']; ?>">
            </div>
            <div class='p5 ui-corner-all frameL ml10 border0 w250' style='float:left;'>
                <div class="input-group input-group-sm w100p">
            ...
            <div class='p5 ui-corner-all frameL ml10 border0 w430 h530' style='float:left;'>
				<a href="#" class="thumbnail w100p h450 mb5">
					<img id="img_good" style="max-height: 440px; max-width: 400px;" src="<?php echo ($row['ImageURL']==null)?'':'../images_goods/'.$row['ImageURL'];?>" alt="">
				</a>
